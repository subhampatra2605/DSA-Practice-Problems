...
* If heap size exceeds k → remove least frequent number
* Remaining elements are the top k frequent

---

## ✔️ **Why Min-Heap Works**
...
# 🧩 **Java Code (Optimal – Priority Queue / Min-Heap)**
...
class Solution {
    public int[] topKFrequent(int[] nums, int k) {
        ...
        for (int n : nums) {
            count.put(n, count.getOrDefault(n, 0) + 1); // Give me the current frequency of n if it exists in the map. If it does NOT exist, give me 0. We increment the count because we saw the number one more time.
        }

        // 2) Min-heap based on frequency (smallest freq at top)
        Queue<Integer> heap = new PriorityQueue<>(
            (a, b) -> count.get(a) - count.get(b) // It tells Priority Queue Compare two elements a and b by their frequency in the count map.
        );
